...
export const getRankings = async (mazeIds: string[]): Promise<RankingResult> => {
    ...
    // Get all of the mazes
    const mazes: { [key: string]: Maze } = {};
    for (const mazeId of mazeIds) {
        mazes[mazeId] = await MazeService.getMazeById(mazeId);
    }

    // Get the participants from the actions table
    const participants = await pgQuery(`
    ...
    */
    ...
    if (mostRevisitedCell.length != 0) {
        const mostRevisitedCellAward: Award = {
            name: "Favorite Spot",
            description: "The rat who revisited the same cell the most",
